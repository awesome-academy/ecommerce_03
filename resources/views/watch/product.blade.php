                    ...
                    <tbody>
                        @foreach ($products as $product)
                            <tr>
                                ...
                                <td>
                                    <a href="javascript:;" class="flex-c-m size1 bg4 bo-rad-23 hov1 s-text1 trans-0-4" id="{{route('product.addcart')}}" {{ Auth::check()? 'onclick=addcart('.$product->id.')': '' }}>@lang('message.add_to_cart')
                                        {{ Form::hidden('name', route('product.addcart'), array('id' => $product->id)) }}
                                    </a>
                                </td>
                            </tr>
                        @endforeach
